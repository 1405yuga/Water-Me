...
class PlantViewModel(application: Application) : ViewModel() {
    ...
    internal fun scheduleReminder(
        ...
        //  instance
        //var reminder = workManager.beginUniqueWork(plantName,ExistingWorkPolicy.REPLACE,
            //OneTimeWorkRequest.from(WaterReminderWorker::class.java))

        val builder = OneTimeWorkRequestBuilder<WaterReminderWorker>().setInputData(data).setInitialDelay(duration,unit)
        //val reminder = workManager.beginUniqueWork(plantName,ExistingWorkPolicy.REPLACE,builder.build())

        //  Enqueue the request as a unique work request
        workManager.beginWith(builder.build()).enqueue()
